...
def ensure_history_table(conn):
    ...
    Also adds missing columns for backward compatibility with older schema
    that used 'name' as the unique key instead of 'version'.
    """
    with conn.cursor() as cur:
        # Create with full schema if the table does not exist at all
        cur.execute("""
        ...
        # Back-compat: table may already exist with an older schema (no 'version' col).
        # Detect which columns are actually present and add any that are missing.
        cur.execute("""
        ...
        if 'version' not in existing_cols:
            # Add as nullable first -- existing rows would violate NOT NULL otherwise
            cur.execute("ALTER TABLE migration_history ADD COLUMN version VARCHAR(50);")
            # Populate from 'name' when it follows the NNNN_slug convention
            if 'name' in existing_cols:
                cur.execute("""
                    UPDATE migration_history
                    SET    version = SUBSTRING(name FROM '^([0-9]{4})')
                    WHERE  version IS NULL AND name ~ '^[0-9]{4}';
                """)
            # Unique index instead of constraint -- allows NULLs without conflict
            cur.execute("""
                CREATE UNIQUE INDEX IF NOT EXISTS migration_history_version_uq
                ON migration_history (version)
                WHERE version IS NOT NULL;
            """)

        if 'rollback_at' not in existing_cols:
            cur.execute(
                "ALTER TABLE migration_history ADD COLUMN rollback_at TIMESTAMPTZ;"
            )
...
def cmd_rabbit_cleanup(confirm: bool = False) -> bool:
    ...
    try:
        ...
        print(f"⚠️   pika/httpx not installed ({e}) — RabbitMQ cleanup skipped."); return True

    host      = os.getenv("RABBITMQ_HOST",            "localhost")
    amqp_port = int(os.getenv("RABBITMQ_PORT",        "5672"))
    mgmt_port = int(os.getenv("RABBITMQ_MANAGEMENT_PORT", "15672"))
    user      = os.getenv("RABBITMQ_USER",             "pulse")
    password  = os.getenv("RABBITMQ_PASSWORD",         "pulse")
    vhost     = os.getenv("RABBITMQ_VHOST",            "pulse_etl")
