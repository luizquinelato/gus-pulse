...
def apply(connection):
    ...
    try:
        ...
        # Check if Google tenant already exists
        cursor.execute("SELECT id FROM tenants WHERE name = 'Google' LIMIT 1;")
        existing_tenant = cursor.fetchone()

        if existing_tenant:
            print(f"   ℹ️ Google tenant already exists (ID: {existing_tenant[0]}), updating tier to premium...")
            cursor.execute("""
                UPDATE tenants
                SET tier = 'premium',
                    website = 'https://www.google.com',
                    assets_folder = 'google',
            ...
            print("   ➕ Creating new Google tenant...")
            cursor.execute("""
...
def rollback(connection):
    ...
    try:
        ...
        cursor.execute("DELETE FROM workflows_steps WHERE tenant_id IN (SELECT id FROM tenants WHERE name = 'Google');")
